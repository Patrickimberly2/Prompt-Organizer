...
class PromptExtractor:
    ...
    def _extract_from_child_pages(self, blocks: List[Dict], source: str, category: str, depth: int) -> List[Prompt]:
        ...
        if child_pages:
            print(f"  {'  ' * depth}Found {len(child_pages)} child pages, processing...")

        for i, block in enumerate(child_pages):
    ...
    def _extract_from_databases(self, blocks: List[Dict], source: str, category: str, depth: int) -> List[Prompt]:
        ...
        if databases:
            print(f"  {'  ' * depth}Found {len(databases)} database(s), processing...")
    ...
    def _extract_from_database(self, database_id: str, source: str, category: str, depth: int) -> List[Prompt]:
        ...
        print(f"  {'  ' * depth}    Database contains {len(pages)} entries")

        for idx, page in enumerate(pages):
            if idx % 10 == 0 and idx > 0:
                print(f"  {'  ' * depth}    Processed {idx}/{len(pages)} database entries...")

            # Extract prompt from database page properties
            prompt = self._extract_from_database_page(page, source, category)
            if prompt:
    ...
    def _extract_from_database_page(self, page: Dict, source: str, category: str) -> Optional[Prompt]:
        """Extract a prompt from database page properties"""
        ...
        # Try to find prompt text in various property names
        for prop_name, prop_value in properties.items():
            ...
            if prop_type == "title":
                ...
                prompt_name = self.notion.extract_text_from_rich_text(title_array)

            # Get rich text (often contains the prompt)
            elif prop_type == "rich_text":
                text_array = prop_value.get("rich_text", [])
                text = self.notion.extract_text_from_rich_text(text_array)
                if text and len(text) > 20:  # Only use substantial text
                    if not prompt_text or len(text) > len(prompt_text):
                        prompt_text = text

        # If we found both name and text, create a prompt
        if prompt_name and prompt_text:
            return Prompt(
                name=prompt_name[:100] + "..." if len(prompt_name) > 100 else prompt_name,
                prompt_text=prompt_text,
                prompt_type="database-entry",
                source=source,
                categories=[category] if category else []
            )
        elif prompt_name:  # Use name as both name and text if that's all we have
            return Prompt(
                name=prompt_name[:100] + "..." if len(prompt_name) > 100 else prompt_name,
...
class MigrationOrchestrator:
    ...
    def migrate_ultimate_chatgpt_bible(self, root_page_id: str):
        ...
        for block in children:
            ...
            print(f"   OK Inserted {inserted} prompts")
    ...
    def migrate_collection(self, collection_name: str, root_page_id: str):
        """Generic collection migration with recursive extraction"""
        print(f"\n{'='*60}")
...
def main():
    ...
    for collection_name, page_id in MIGRATIONS.items():
        try:
            ...
            print(f"ERROR: Failed to migrate {collection_name}: {e}")
            continue
